...
const historical = async () => {
  try {
    ...
    const response = await request.post("/horizontal/horizontalSheetCageHistoryTask/queryTaskHistory", {
      pageNo: 1,
  ...
  try {
    ...
    if (selectValuesa[0] != null && selectValuesa[0] != 'undefined') {
      if (selectValuesa[0] != "") {
        ...
        taskState = Number(selectValuesa[0]);
      }
    }
...
const setHistorical = async () => {
  try {
    let celllist = []
    if (selectValuesa[0] != null && selectValuesa[0] != 'undefined' && selectValuesa[0] != "") {
      // celllist = [selectValuesa[0]];
        taskState = Number(selectValuesa[0]);
    } else {
      taskState = '';
    }
    console.log('Current selectValuesa:', selectValuesa);
    let startTime = window.localStorage.getItem('startTime')
    let page = window.localStorage.getItem('pagenumber')
    const response = await request.post("/horizontal/horizontalSheetCageHistoryTask/queryTaskHistory", {
      pageNo: 1,
...
function getStatusText(taskState) {
  switch (taskState) {
    ...
    case 3:
      return t('sorter.straightTasks');//任务失败
    case 7:
  ...
  <div>
    <div style="display: flex;width: 1770px;">
      <el-select v-model="selectValuesa[0]" :placeholder="$t('Mounting.taskStatus')" clearable filterable
        ...
        <el-option :label="$t('Mounting.inAction')" value="0"></el-option>
        <el-option :label="$t('film.missionAccomplished')" value="1"></el-option>
        <el-option :label="$t('film.taskFailed')" value="3"></el-option>
        <el-option :label="$t('film.taskFailed')" value="8"></el-option>
      </el-select>
      <el-date-picker v-model="timeRange" :end-placeholder="$t('reportmanage.endtime')" :shortcuts="shortcuts"
                      :start-placeholder="$t('reportmanage.starttime')" range-separator="至" style="margin-left: 15px;"
                      type="datetimerange"/>
    ...
    <el-table ref="table" :data="tableDatax" :header-cell-style="{ background: '#F2F3F5 ', color: '#1D2129' }"
              style="margin-top: 20px;height: 580px;width: 1770px;">
      <el-table-column :label="$t('Mounting.glassID')" align="center" min-width="70" prop="glassId"/>
      <el-table-column :label="$t('Mounting.width')" align="center" min-width="70" prop="width"/>
      ...
      <el-table-column :label="$t('workOrder.line')" align="center" min-width="80" prop="workLine"/>
      ...
      <!-- <el-table-column :label="$t('searchOrder.taskType')" align="center" min-width="80" prop="taskType"/> -->
      <el-table-column :label="$t('film.startingGrid')" align="center" min-width="80" prop="startSlot"/>
      <el-table-column :label="$t('film.targetGrid')" align="center" min-width="80" prop="targetSlot"/>
      <el-table-column :label="$t('Mounting.taskStatus')" align="center" min-width="50" prop="taskState">
        <template #default="scope">
