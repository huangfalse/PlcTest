...
const fetchData = async () => {
  try {
    ...
    const firstKey = Object.keys(result.data.tempAfterQueueInfo)[0]
    if (firstKey && result.data.tempAfterQueueInfo[firstKey]) {
      processTemperData(result.data.tempAfterQueueInfo[firstKey])
    }
  } catch (error) {
  }
}
...
const handleMessage = (data) => {
  if (data?.tempAfterQueueInfo) {
    const firstKey = Object.keys(data.tempAfterQueueInfo)[0]
    if (firstKey && data.tempAfterQueueInfo[firstKey]) {
      processTemperData(data.tempAfterQueueInfo[firstKey])
    }
  }
}
// 显示对话框
...
const handleDamageOut = async () => {
  ...
  try {
    ...
    updateRectColorsOut()
    // 强制刷新视图（如果需要）
    // adjustedTemper.value = [...adjustedTemper.value]
  } catch (error) {
  }
}
onMounted(() => {
