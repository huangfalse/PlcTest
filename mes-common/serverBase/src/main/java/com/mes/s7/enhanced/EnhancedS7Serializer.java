 ...
 * @Description:
 */
public class EnhancedS7Serializer implements IPLCSerializable {
    private final S7PLC s7PLC;

    public EnhancedS7Serializer(S7PLC s7Plc) {
        this.s7PLC = s7Plc;
    }
     ...
     */
    public static EnhancedS7Serializer newInstance(S7PLC s7Plc) {
        return new EnhancedS7Serializer(s7Plc);
    }
     ...
     */
    private List<S7ParseData> parseBean(final Class<?> targetClass, String dbArea, int beginIndex) {
        ...
        for (final Field field : targetClass.getDeclaredFields()) {
            ...
            int index = Integer.parseInt(s7Variable.address()) + beginIndex;
            String address = dbArea + index;
            S7Parameter parameter = new S7Parameter(address, s7Variable.type(), s7Variable.count());
     ...
     * @return
     */
    public <T> T read(Class<T> targetClass, String dbArea, int beginIndex) {
        List<S7ParseData> s7ParseDataList = this.parseBean(targetClass, dbArea, beginIndex);
     ...
     * @param <T>
     */
    public <T> void write(T targetBean, String dbArea, int beginIndex) {
        // 解析参数
        List<S7ParseData> s7ParseDataList = this.parseBean(targetBean.getClass(), dbArea, beginIndex);
